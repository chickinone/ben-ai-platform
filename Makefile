...
.PHONY: help init venv lint fmt test test-integration seed-demo eval-pii config up up-core down ps logs migrate pull-model
...
venv:        ## Tạo .venv và cài package editable
	...
	$(PY) -m pip install -e libs/ben_common -e libs/ben_telemetry -e libs/pii_vn -e "services/gateway[dev]" -e "services/control-plane[dev]" -e "services/metering-worker[dev]" -e "libs/ben_litellm_plugins[dev]" ruff mypy openai anthropic
